...
try /* Блок обработки исключений. Если ввели некорректные данные, то смотри строку 40*/
{
    ...
    if (A >= 1) /* Если вводимое число больше единицы, то...*/
    {
        ...
        {
            sum += i; /* сумму будем увеличивать на значение счётчика,
                         т.е. сначала +1, потом +2, потом +3 и т.д. 
                         (sum += i)- это то же самое, что и (sum = sum + i).*/
        }
        Console.WriteLine($"Сумма чисел от 1 до {A} равна {sum}"); /* В ответ пойдёт сумма от последнего действия.*/
    }
    ...
    if (A < 1) /* Если вводимое число меньше единицы, то...*/
    {
        ...
        {
            sum += i; /* к сумме будем прибавлять  значение счётчика,
